...
MockMvcRequestSpecification request = given()
.header("Content-Type", "application/json")
.body("[{\"type\":\"AIRCRAFT_CARRIER\",\"id\":1,\"placement\":{\"x\":9,\"y\":6},\"orientation\":\"DOWN\"}]");
...
// and:
DocumentContext parsedJson = JsonPath.parse(response.getBody().asString());
assertThatJson(parsedJson).array("errors").array("validations").contains("code").isEqualTo("OrientationCheck");
...
assertThatJson(parsedJson).array("errors").array("validations").array("value").field("placement").field("x").isEqualTo(9);
...
assertThatJson(parsedJson).array("errors").array("validations").contains("message").isEqualTo("Incorrect orientation.");
assertThatJson(parsedJson).array("errors").array("validations").array("value").field("placement").field("y").isEqualTo(6);
