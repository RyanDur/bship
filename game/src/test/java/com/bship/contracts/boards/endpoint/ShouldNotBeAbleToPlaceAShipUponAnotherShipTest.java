...
MockMvcRequestSpecification request = given()
.header("Content-Type", "application/json")
.body("{\"type\":\"AIRCRAFT_CARRIER\",\"id\":1,\"start\":{\"x\":3,\"y\":3},\"end\":{\"x\":7,\"y\":3}}");
...
assertThatJson(parsedJson).array("pieces").field("end").field("x").isEqualTo(7);
...
assertThatJson(parsedJson).array("pieces").field("start").field("y").isEqualTo(3);
...
MockMvcRequestSpecification request = given()
.header("Content-Type", "application/json")
.body("{\"type\":\"BATTLESHIP\",\"id\":2,\"start\":{\"x\":4,\"y\":2},\"end\":{\"x\":4,\"y\":5}}");
...
assertThatJson(parsedJson).array("errors").array("validations").contains("code").isEqualTo("ShipCollisionCheck");
assertThatJson(parsedJson).array("errors").array("validations").contains("message").isEqualTo("Ship collision.");
}
